...
class DINOv3DatasetConfig(NVDINOv2DatasetConfig):
    ...
    train_manifest: Optional[str] = STR_FIELD(
        ...
        default_type=None,
        description=(
            "Optional Parquet manifest with sample_id, storage_type (file|tar|zip), path, "
            "and member for archive rows. Paths are absolute or relative to images_dir "
            "(the manifest directory when images_dir is empty). Optional replay_repeat "
            "is a nonnegative, contiguous repeat index starting at zero per sample. When "
